...
sub handle_incoming_file {
    ...
    if ($tags) {
        ...
        foreach my $t (@tags) {
            ...
            if ( $t =~ /source:(.*)/i ) {
                my $url = $1;
                $logger->debug("将 $url 添加为 $id 的 URL。");
                trim_url($url);
    ...
    if ($catid) {
        $logger->debug("将上传的文件添加到分类 $catid");
...
sub download_url {
    ...
    $logger->info("正在下载 URL $url... 这将需要一些时间。");
    ...
    $logger->debug("内容处置头: $content_disp");
