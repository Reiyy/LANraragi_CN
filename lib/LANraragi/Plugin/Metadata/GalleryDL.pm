...
sub plugin_info {
    ...
    return (
        ...
        version     => "1.0",
        description =>
            "收集gallery-dl生成的info.json文件中的元数据，该文件可内嵌于档案中或与案同目录且同名（{archive_name}.json）",
        parameters => []
    );
}
...
sub get_tags {
    ...
    if ($path_in_archive) {
        ...
        $logger->debug("在档案 $filepath 中发现文件");
        $delete_after_parse = 1;
    } elsif ( -e $path_nearby_json ) {
        $filepath = $path_nearby_json;
        $logger->debug("在 $filepath 附近发现文件");
        $delete_after_parse = 0;
    } else {
        die "未找到档案内嵌info.json或与档案同名的{archive_name}.json文件！\n";
    }
    ...
    #Return tags
    $logger->info("正在将以下标签发送至LRR：$tags");

    if ($title) {
        $logger->info("解析标题为 $title");
