...
sub plugin_info {
    ...
    return (
        ...
        author      => "psilabs-dev",
        version     => "0.1",
        description => "通过作品ID获取Pixiv艺术作品的元数据。
            <br>支持从以下文件格式中提取 ID：\"{Id} Title\" or \"pixiv_{Id} Title\"。
            <br>
            <br><i class='fa fa-exclamation-circle'></i> Pixiv对API请求施加了速率限制，过度使用可能导致账号被暂停或封禁。
        ",
        ...
        parameters  => [
            { type => 'string', desc => '支持的语言列表，使用逗号分隔。选项：jp, en。空字符串默认为仅使用原始标签（jp）。' }
        ],
        cooldown    => 1
    );
