...
sub plugin_info {
    ...
    return (
        ...
        author      => 'Mayriad',
        version     => '1.1.1',
        description => '使用由 Mayriad 的 EH Master Script 下载的档案文件名中的标识符，'
          . '准确地从 e-hentai.org 获取元数据。',
        ...
        parameters => [
            {   type => 'bool',
                desc => '在可用时保存原始日文标题，而不是英文或' . '罗马音标题'
            },
            { type => 'bool', desc => '获取额外时间戳（发布时间）和上传者元数据' },
            { type => 'bool', desc => '使用 ExHentai 链接作为来源，而不是 E-Hentai 链接' }
        ],
        oneshot_arg => '输入有效的 EH 画廊URL，将该 EH 画廊的元数据复制到这个 LANraragi 档案中',
        cooldown    => 4
    );
}
