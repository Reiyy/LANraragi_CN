...
sub plugin_info {
    ...
    return (
        ...
        parameters => [
            { type => "string", desc => "在搜索中强制使用的语言（由于 EH 限制，日语无法使用）" },
            { type => "bool",   desc => "优先使用缩略图获取（否则使用标题）" },
            { type => "bool",   desc => "使用标题中的 gID 进行搜索（否则使用标题）" },
            { type => "bool",   desc => "使用 ExHentai（启用此功能以在没有star Cookie的情况下搜索fjorded内容）" },
            {   type => "bool",
                desc => "保存原始标题（如果可用），而不是英文或罗马音标题"
            },
            { type => "bool", desc => "获取额外时间戳（发布时间）和上传者元数据" },
...
sub lookup_gallery {
    ...
    if ( $search_gid && $title_gid ) {
        ...
        $logger->debug("找到gID：$title_gid，使用URL $URL（来自档案标题的gID）");
...
sub ehentai_parse() {
    ...
    if ( index( $dom->to_string, "You are opening" ) != -1 ) {
        ...
        $logger->info("由于“EH过多请求警告”，正在等待 $rand 秒。");
        sleep($rand);
    }
...
sub get_tags_from_EH {
    ...
    my $ehtags = join( ', ', @tags );
    $logger->info("正在将以下标签发送到LRR：$ehtags");
