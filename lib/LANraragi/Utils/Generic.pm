...
sub start_minion {
    ...
    $logger->info("在子进程中启动新的 Minion 工作线程，并行工作数量为 $numcpus。");
    ...
    $proc->start(
        sub {
            $logger->info("Minion 工作线程 $$ 已启动");
            $worker->run;
            $logger->info("Minion 工作线程 $$ 已停止");
            return 1;
        }
    );
...
sub _spawn {
    ...
    $job->app->log->debug(qq{进程 $pid 正在执行任务 "$task"，工作 "$id"});
}
