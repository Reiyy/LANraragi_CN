...
sub plugin_info {
    return (
        ...
        version     => "0.3",
        description => "收集嵌入到你的档案中的元数据作为 Chaika 风格的 api.json 文件",
        icon        =>
        ...
        parameters => [
            { type => "bool", desc => "添加以下标签(如果可用)：下载URL、画廊ID、分类、时间戳" },
            {   type => "bool",
                desc =>
                  "将没有命名空间的标签添加到“other:”命名空间中，而不是默认命名空间，这与 E-H 的命名空间处理方式一致"
            },
            {   type => "string",
                desc => "将自定义“source:”标签添加到您的归档。例如：chaika。如果为空，则不会添加标签"
            }
        ],
    );
}
